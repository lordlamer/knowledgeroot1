    ...
    function _writeAttrs ($attrs) 
    {
        if (is_array($attrs)) {
            foreach ($attrs as $name => $value) {
                ...
                $tempval = preg_replace_callback('/&#(\d+);?/m', function ($matches) { return char($matches[0]); }, $value);
                $tempval = preg_replace_callback('/&#x([0-9a-f]+);?/mi', function ($matches) { return chr(hexdec($matches[0])); }, $tempval);
